...
def save_objects(file_name, file_ext, bboxs, confs, classIds, classNames,desired_width,desired_height):
    ...
    for i in range(len(bboxs)):
        ...
        df.at[j,CL[0]] = os.path.basename(file_name+file_ext)
        df.at[j,CL[1]] = j+1
    ...
    df_GUI.to_csv(file_name+'.txt', header=None, index=None, sep=' ')
    df_GUI.to_csv(file_name + '_VAL.txt', header=None, index=None, sep=' ')
