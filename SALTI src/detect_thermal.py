...
def detect(save_txt=False, save_img=False):
    ...
    for path, img, im0s, vid_cap in dataset:
        ...
        # Set the directory to store the results file
        ROOT_DIR = os.path.abspath(os.curdir)
        sys.path.append(ROOT_DIR)
        STORE_DIR = os.path.join(ROOT_DIR, "Save_thermal")
            ...
            if det is not None and len(det):
                ...
                for *xyxy, conf, _, cls in det:
                    ...
                    if save_img or view_img:  # Add bbox to image
                        ...
                        t_con = np.squeeze(xyxy)
            #
            #             # Storing each picture's results in its dataframe
            #
            #             df[CL[0]][j] = str(Path(p).name)
            #
            #             df[CL[1]][j] = j + 1
            #             df[CL[2]][j] = int((t_con[0]+t_con[2])/2)
            #             df[CL[3]][j] = int((t_con[1]+t_con[3])/2)
            #             df[CL[4]][j] = abs(t_con[1]-t_con[3])
            #             df[CL[5]][j] = abs(t_con[0]-t_con[2])
            #             df[CL[6]][j] = str(classes[int(cls)])
            #             df[CL[7]][j] = float(conf)
            #
            #             j = j+1
            #
            # # Exporting each picture's results to its specific csv file
            # df.to_csv(os.path.join(STORE_DIR, str(Path(p).name) + '.csv'), index=False)
...
#     #parser.add_argument('--weights', type=str, default='Yolo_config/yolov3-spp.weights', help='path to weights file')
#     parser.add_argument('--source', type=str, default=os.path.join(dir_dataset,dir_thermal), help='source')  # input file/folder, 0 for webcam
...
if __name__ == '__main__':
    print('data/coco.data')
    print(cfg_T.dir_classes)
    print(type('data/coco.data'))
    print(type(cfg_T.dir_classes))
